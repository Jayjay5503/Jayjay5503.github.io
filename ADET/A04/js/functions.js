...
function loadProducts(categoryIndex) {
    ...
    contents.forEach(function (content) {
        if (content.isAvailable) {
            maincontainer.innerHTML += `
        <div onclick="addToReceipt(` + content.price + `, '` + content.name + `')" class="card mx-1 my-2 custom-button content p-3 text-center" style="width: 180px; background-color:rgb(50, 68, 116)">
          <img src="img/` + content.image + `.png" class="img-fluid" style="height: 200px; width: 150px; object-fit: contain; margin: 0;">
          <small style="color: #c0b300; font-size: 18px; font-weight: bold;">` + content.name + `</small><br>
          <small style="color: #c0b300; font-size: 18px;">₱` + content.price + `</small>
        </div>
      `;
        }
    });
...
function confirmPurchase() {
    ...
    if (hasItems) {
        purchaseMessage.innerText = "Your order is in!\nThanks for choosing us, we're cooking it up with care!";
        receiptItems = [];
        total = 0;
        updateReceipt();
    } else {
